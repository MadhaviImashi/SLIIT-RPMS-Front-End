...
const RequestCard = ({ group, topic, description, requestId, reFetch }) => {
  ...
  const handleAccept = () => {
    setIsLoading(true);
    fetch("http://localhost:4000/api/researchGroup//accept-cosupervisor", {
      method: "POST",
      body: JSON.stringify({
        requestId,
      }),
      ...
      .then((result) => {
        if (result.message === "Request accepted successfully") {
          setIsMessage(true);
          setIsLoading(false);
      ...
      .catch((err) => {
        setIsMessage(true);
        setIsLoading(false);
        setMessage("Request accept failed");
      });
  };

  const handleReject = () => {
    setIsLoading(true);
    fetch("http://localhost:4000/api/researchGroup/decline-cosupervisor", {
      method: "POST",
      body: JSON.stringify({
        requestId,
      }),
      ...
      .then((result) => {
        if (result.message === "Request declined successfully") {
          setIsMessage(true);
          setIsLoading(false);
      ...
      .catch((err) => {
        setIsMessage(true);
        setIsLoading(false);
        setMessage("Request decline failed");
      });
  };
